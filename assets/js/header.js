jQuery(function($) {
    ...
    $nav.find('.__dim__').on('click', function(e) {
        e.stopPropagation();
        $('.__btn-nav-toggle').trigger('click');
    });
    ...
    $('#nav .__gnb .__menu > li').on('mouseenter', function() {
        ...
        if ($depth2[0]) {
            ...
            gsap.set($('.__dim__', $depth2), {
                clearProps: 'all'
            });
            gsap.set($('> ul', $depth2), {
                clearProps: 'all'
            });
            gsap.set($('> ul > li', $depth2), {
                clearProps: 'all'
            });
    ...
    $('#nav .__gnb .__menu > li > a').on('click', function(e) {
        ...
        if (depth1[0]) {
            ...
            if ($(this).closest('li').hasClass('__open')) {
                depth1.tl.set(depth1, {
                    height: 'auto'
                })
                depth1.tl.from(depth1, {
                    height: 0,
                    duration: .25,
                    ease: Quart.easeInOut
                });
            } else {
                depth1.tl.fromTo(depth1, {
                    height: __height
                }, {
                    height: 0,
                    duration: .25,
    ...
    $('.__search .__btn-show', $header).on('click', function(e) {
        ...
        searchBox.tl = gsap.timeline({
                onComplete: function() {
                    ...
                    gsap.set($('.__dim__', searchBox), {
                        clearProps: 'all'
                    });
                    gsap.set($('.__box', searchBox), {
                        clearProps: 'all'
                    });
                }
            })
            ...
            .fromTo($('.__dim__', searchBox), {
                autoAlpha: 0
            }, {
                autoAlpha: 1,
                duration: .25
            })
            .from($('.__box', searchBox), {
                height: 0,
                duration: .4,
                ease: Quart.easeInOut
            })
    ...
    $('.__btn-toggle--key-info, .__key-info .__btn-close, .__key-info .__dim__').on('click', function(e) {
        ...
        keyInfo.tl = gsap.timeline({
            onComplete: function() {
                ...
                gsap.set($('.__dim__', keyInfo), {
                    clearProps: 'all'
                });
                gsap.set($('.__key-info-header', keyInfo), {
                    clearProps: 'all'
                });
                gsap.set($('.__key-info-body', keyInfo), {
                    clearProps: 'all'
                });
                gsap.set($('.__widget', keyInfo), {
                    clearProps: 'all'
                });
                gsap.set($('.__box', keyInfo), {
                    clearProps: 'all'
                });
            }
        })
        ...
        if ($keyInfo.hasClass('__active')) {
            keyInfo.tl
                .set($('.__key-info-body', keyInfo), {
                    autoAlpha: 0
                })
                .fromTo($('.__dim__', keyInfo), {
                    autoAlpha: 0
                }, {
                    autoAlpha: 1,
                    duration: .3
                }, 0.1)
                .fromTo($('.__box', keyInfo), {
                    autoAlpha: 0
                }, {
                    autoAlpha: 1,
                    duration: .3
                ...
                .from($('.__key-info-body', keyInfo), {
                    height: 0,
                    duration: .4,
                    ease: Quart.easeInOut
                }, .3)
                .fromTo($('.__widget', keyInfo), {
                    autoAlpha: 0,
                    y: '-30px'
                }, {
                    autoAlpha: 1,
            ...
            keyInfo.tl
                ...
                .to($('.__key-info-body', keyInfo), {
                    height: 0,
                    duration: .3,
                    ease: Quart.easeInOut
                }, 0)
                .to($('.__key-info-header', keyInfo), {
                    autoAlpha: 0,
                    duration: .3
                }, 0)
                .to($('.__box', keyInfo), {
